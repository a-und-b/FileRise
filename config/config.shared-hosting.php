<?php
// config.shared-hosting.php
// Shared hosting compatible version of config.php
...
// Check if we should use shared hosting mode
$useSharedHosting = (
    ini_get('open_basedir') !== '' ||                    // open_basedir is set
    !is_writable('/var/www/') ||                         // Can't write to standard location
    stripos(__DIR__, '/home/') !== false ||              // Common shared hosting path
    stripos(__DIR__, '/public_html') !== false ||        // cPanel path
    stripos(__DIR__, '/httpdocs') !== false ||           // Plesk path
    getenv('FILERISE_SHARED_HOSTING') === 'true'        // Explicitly enabled
);

if ($useSharedHosting) {
    // Load shared hosting compatibility layer
    require_once dirname(__DIR__) . '/src/SharedHosting/bootstrap.php';
    // Constants are now defined by bootstrap.php
} else {
    // Standard FileRise configuration
...
// Common constants (same for both modes)
...
if (AUTH_BYPASS) {
    ...
    if (!empty($_SERVER[$hdrKey])) {
        ...
        // Use shared hosting safe include if available
        if ($useSharedHosting && function_exists('FileRise\SharedHosting\safeInclude')) {
            require_once \FileRise\SharedHosting\safeInclude('src/models/AuthModel.php');
        } else {
